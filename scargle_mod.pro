PRO scargle_mod,t,c,om,px,fmin=fmin,fmax=fmax,nfreq=nfreq,     $
            ...
            debug=debug,slow=slow, Phi_FT=Phi_FT
;+
; NAME:
;         scargle
;
...
;          Version 1.7, 2018.03.20 VH: 
;               Addition of the keyword Phi_FT (see eq (10) in Hocke and Kampfer, 2009)
;          
;          
;-
   ...
   IF keyword_set(old) THEN BEGIN ;; slow version
       ...
       Phi_FT =  atan(total(cn*si), total(cn*co)) + tau + om*0.5*(time[0]+time[-1])
       
       ;omtau ; om*0.5*(time[0]+time[-1])
   ...
   Phi_FT =  atan(sh, ch) + omtau ; om*0.5*(time[0]+time[-1])
