...
tot_count_w = total(detector_counts,1)
min_cts = min(tot_count_w[where(tot_count_w ne 0., /null)])
...
for it = 0, n_t-1 do begin
  ...
  cgimage, imscl,  position = [0.15, 0.5, 0.8, 0.9], xvector=wvl, yvector=findgen(n_pix), /axes, xt='Wavelength [nm]', yt='Detector Y', title='Total countrate : '+strtrim(string(total(detector_counts[*,*,it]), f='(i10)'),2)+' Hz', charsize=csz-0.6
